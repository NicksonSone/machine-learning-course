...
% one-hot encode y
y = sparse(1:m, y, 1, m, num_labels);

% forward propagation
z1 = a1 = [ones(m, 1) X]; 	% add intercept term
...
% cost function
cost_matrix =  -y .* log(output) - (1 - y) .* log(1-output);
J = sum(sum(cost_matrix, 2)) / m;

% compute regularized term for cost function
reg_term = lambda * (sum(sum(Theta1(:, 2:end).^2, 2)) + sum(sum(Theta2(:, 2:end).^2, 2))) / (2*m);
